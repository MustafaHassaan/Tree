<script lang="ts">
  ...
  let { isOpen = $bindable(false), onClose, onEditEmployee } = $props();

  let employees = $state<any[]>([]);
  let commissions = $state<{ id: number; percentage: number }[]>([]);
  ...
  function getCommissionPercentage(commissionId: number | undefined) {
    if (!commissionId) return 'None';
    const commission = commissions.find(c => c.id === commissionId);
    return commission ? `${commission.percentage}%` : 'None';
  }
  ...
  let totalPages = $derived(Math.ceil(employees.length / itemsPerPage));
  let startIndex = $derived((currentPage - 1) * itemsPerPage);
  let endIndex = $derived(startIndex + itemsPerPage);
  let paginatedEmployees = $derived(employees.slice(startIndex, endIndex));
</script>
            ...
            <table class="w-full">
              <thead>
                <tr class="border-b border-gray-200">
                  ...
                  <th class="text-center py-3 px-4 text-xs font-semibold text-gray-700 uppercase tracking-wider">Role</th>
                  <th class="text-center py-3 px-4 text-xs font-semibold text-gray-700 uppercase tracking-wider">Phone</th>
                  <th class="text-center py-3 px-4 text-xs font-semibold text-gray-700 uppercase tracking-wider">Commission</th>
              ...
              <tbody>
                {#each paginatedEmployees as employee (employee.id)}
                  <tr class="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                    <td class="py-3 px-4 text-sm text-gray-900 font-medium text-center">{employee.name}</td>
                    <td class="py-3 px-4 text-sm text-gray-600 text-center">{getRoleLabel(employee.role)}</td>
                    <td class="py-3 px-4 text-sm text-gray-600 text-center">{employee.phone}</td>
                    <td class="py-3 px-4 text-sm text-gray-600 text-center">
                      {getCommissionPercentage(employee.commissionId)}
                    </td>
